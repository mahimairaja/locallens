...
locallens doctor --format json 2>/dev/null | python -c "import sys, json; d=json.load(sys.stdin); print(f'Doctor: {len(d[\"checks\"])} checks')" || echo "Doctor: ran (some checks failed as expected)"

echo "6. Installing MCP extra..."
pip install -e ".[mcp]" --quiet 2>&1 | tail -3
